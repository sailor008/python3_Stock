...
# return dic data for json
# 【注意：需要处理网络连接失败的逻辑】
def requestUrlWithGet(url, req_params, req_headers = None):
	global m_ctx
	# 拼接url
	Logger.log("Method.GET request.url = "+url)

	urlReq = request.Request(
		url,
	...
	if req_headers != None:
		for key, value in req_headers.items():
			urlReq.add_header(key, value)

	response = request.urlopen(urlReq, context=m_ctx)
	respData = response.read().decode('utf-8')
	resultJson = json.loads(respData)
	return resultJson


# return dic data for json
# 【注意：需要处理网络连接失败的逻辑】
def requestUrlWithPost(url, req_params, req_headers = None):
	global m_ctx
	# 拼接url
	Logger.log("Method.POST request.url = "+url)
...
def SendRequest_API_TuShare(api_name, fields='', **kwargs):
	req_params = {
        'api_name': api_name,
        'token': constant.TuShare_Token,
        'params': kwargs,
        'fields': fields, 
	}
	print(fields)
	resultJson = requestUrlWithPost("http://api.waditu.com", req_params, None)
	if resultJson['code'] == 0:
		resultData = resultJson["data"]
		columns = resultData['fields']
